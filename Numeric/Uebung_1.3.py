...
i = 1
while (abs(intervall(0,1,i,curve)-result)/result) > precision:
    i = i + 1
...
print("i)\t" + str(singlePrec))

i = 1
while (abs(intervall(0,1,i,mass)-m)/m) > precision:
    i = i + 1
doublePrec = intervall(0,1,(i-1)*2,mass)

print("ii)\t" + str(doublePrec))
print("Faktor: " + str((singlePrec-m) / (doublePrec-m)))
